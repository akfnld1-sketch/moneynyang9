...
function renderEtcCalendar(){
  ...
  for(let d = 1; d <= dim; d++){
    ...
    etcItems.slice(0, 2).forEach(it => {
      html += `<div style="font-size:9px;color:#3dd68c;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">✨ ${it.name||'추가수익'}</div>`;
    });
    if(etcItems.length > 2) html += `<div style="font-size:8px;color:var(--text3);">+${etcItems.length-2}건</div>`;
    if(dayAmt > 0) html += `<div style="font-size:9px;color:#3dd68c;font-weight:700;margin-top:1px;">${dayAmt>=10000?Math.round(dayAmt/10000)+'만':dayAmt.toLocaleString()}원</div>`;
  ...
  document.getElementById('stats-row').innerHTML = `
    ...
    <div class="stat-card"><div class="lbl">이번달 수입</div><div class="val" style="color:#3dd68c;font-size:15px;">${totalAmt>=10000?Math.round(totalAmt/10000)+'만':totalAmt.toLocaleString()}원</div></div>`;
}
    ...
    const itemsHTML = etcItems.length > 0
      ? etcItems.map((it,i)=>`
          <div style="display:flex;justify-content:space-between;align-items:center;
            ...
            <div>
              <div style="font-size:13px;font-weight:600;color:var(--text);">✨ ${it.name||'추가수익'}</div>
              ${it.memo?`<div style="font-size:11px;color:var(--text3);">${it.memo}</div>`:''}
            </div>
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:14px;font-weight:700;color:#3dd68c;">+${(it.amount||0).toLocaleString()}원</span>
              <button onclick="deleteNjobEtcByIdx('${key}',${i})"
                style="background:none;border:none;color:var(--text3);font-size:16px;cursor:pointer;padding:2px;">✕</button>
            </div>
          </div>`).join('')
      : `<div style="text-align:center;color:var(--text3);font-size:13px;padding:16px 0;">아직 기록이 없어요</div>`;
    ...
    overlay.innerHTML = `
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:20px;
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
          <div style="font-size:16px;font-weight:700;color:var(--text);">✨ ${curY}년 ${curM+1}월 ${d}일 추가수익</div>
        ...
        ${total>0?`<div style="text-align:right;font-size:13px;font-weight:700;color:#3dd68c;margin-bottom:12px;">합계 +${total.toLocaleString()}원</div>`:''}
        ...
        <div style="background:rgba(61,214,140,.05);border:1px solid rgba(61,214,140,.2);border-radius:10px;padding:12px;">
          ...
          <div id="njob-etc-preview" style="display:none;text-align:right;font-size:14px;font-weight:700;color:#3dd68c;margin-bottom:8px;"></div>
